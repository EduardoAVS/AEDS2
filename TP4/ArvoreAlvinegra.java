...
class ArvoreAlvinegra{
    ...
    static class Arvore{
        ...
        public void inserir(Jogador elemento) {
            ...
            raiz.cor = false; // A raiz deve ser sempre preta
        }
    
        private No inserir(Jogador elemento, No i) {
            if (i == null) {
                return new No(elemento, true); // Novos nós são sempre vermelhos
            }
            ...
            if (elemento.getNome().compareTo(i.elemento.getNome()) < 0) {
                ...
                System.out.println("Elemento inserido já existe!");
                return i;
            }
    
            // Balanceamento após a inserção
            if (ehNoVermelho(i.dir) && !ehNoVermelho(i.esq)) {
                i = rotacionarEsq(i);
            }
            if (ehNoVermelho(i.esq) && ehNoVermelho(i.esq.esq)) {
                i = rotacionarDir(i);
            }
            if (ehNoVermelho(i.esq) && ehNoVermelho(i.dir)) {
                inverterCores(i);
            }
        ...
        private boolean ehNoVermelho(No no) {
