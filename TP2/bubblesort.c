...
int main(void) {
    ...
    //implementando o bolha
    for(int i = 0; i < controle - 1; i++){
        for(int j = 0; j < controle - 1; j++){
           if ((jogadorID[j].anoNascimento > jogadorID[j + 1].anoNascimento) || (jogadorID[j].anoNascimento == jogadorID[j + 1].anoNascimento && strcmp(jogadorID[j].nome, jogadorID[j + 1].nome) > 0)) {
                swap(&jogadorID[j], &jogadorID[j + 1]);
            }
        }
    }
