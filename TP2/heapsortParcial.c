...
typedef struct Jogador{
    ...
    void construir(Jogador jogador[], int tamHeap){
        for(int i = tamHeap; i > 1 && getAltura(&jogador[i]) > getAltura(&jogador[i/2]) || (getAltura(&jogador[i]) == getAltura(&jogador[i / 2]) && strcmp(getNome(&jogador[i]), getNome(&jogador[i / 2])) > 0); i /= 2){
    ...
    void reconstruir(Jogador jogador[], int tamHeap){
        ...
        while(i <= (tamHeap/2)){
            ...
            if(getAltura(&jogador[i]) < getAltura(&jogador[filho]) || (getAltura(&jogador[i]) == getAltura(&jogador[filho]) && strcmp(getNome(&jogador[i]), getNome(&jogador[filho])) < 0)){
